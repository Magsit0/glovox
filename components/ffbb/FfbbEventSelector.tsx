...
import type { FfbbEventOption } from "@/lib/ffbb/types";

interface Props {
  options: FfbbEventOption[];
  selectedId?: string;
}

function matches(option: FfbbEventOption, q: string): boolean {
  if (!q) return true;
  const needle = q.toLowerCase();
  return (
    option.eventoId.toLowerCase().includes(needle) ||
    option.nombre.toLowerCase().includes(needle) ||
    option.categoriaEvento.toLowerCase().includes(needle)
  );
}
...
export default function FfbbEventSelector({ options, selectedId }: Props) {
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        <span className="min-w-0 flex-1 truncate">
          {selected ? (
            <>
              <span className="font-medium text-[#333333]">{selected.eventoId}</span>
              <span className="ml-2 text-[#666666]">{selected.nombre}</span>
              {selected.fechaEvento && (
                <span className="ml-2 text-[#999999]">· {formatFecha(selected.fechaEvento)}</span>
              )}
            </>
          ) : (
            <span className="text-[#999999]">Buscar y seleccionar evento</span>
          )}
        </span>
                ...
                <li key={opt.eventoId} role="option" aria-selected={isActive}>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 truncate">
                      ...
                      {opt.categoriaEvento && (
                        <span className="ml-2 text-xs text-[#999999]">{opt.categoriaEvento}</span>
                      )}
                    </span>
                    {opt.fechaEvento && (
                      <span className="shrink-0 text-xs text-[#999999]">
                        {formatFecha(opt.fechaEvento)}
                      </span>
                    )}
