...
import { budgetCeiling, projectRevenue } from "@/lib/budget-forecast/formulas";
import { createPresupuestoAction } from "@/app/presupuesto/actions";

interface Props {
  presupuestos: PresupuestoEvento[];
  /** Eventos de categoriaEvento que aún no tienen presupuesto. */
  eventosDisponibles: EventoOption[];
}

function resumen(p: PresupuestoEvento): { eventoId: string; ingreso: number; techo: number } {
  const doc = coerceDoc(p.doc);
  const revenue = projectRevenue(doc);
  const techo = budgetCeiling(revenue.total, doc.marginMode, doc.targetMargin);
  return { eventoId: doc.eventoId, ingreso: revenue.total, techo };
}

export default function PresupuestoList({ presupuestos, eventosDisponibles }: Props) {
    ...
    <section className="flex flex-col gap-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 font-sans text-sm text-[#666666]">
            Cada presupuesto se crea a partir de un evento de glovox.categoriaEvento. Los
            per-cápitas y % por categoría se siembran desde el histórico y quedan editables.
          </p>
        </div>
        <button
          type="button"
      ...
      {showForm && (
        <NewPresupuestoForm
          ...
          onCreated={(id) => router.push(`/presupuesto?plan=${id}`)}
        />
      )}
        ...
        <table className="w-full font-sans text-sm">
          <thead>
            <tr className="border-b border-[#E5E5E5] bg-[#FAFAFA]">
              ...
              <th className="px-4 py-3 text-right font-medium text-[#666666]">Ingreso proy.</th>
              <th className="px-4 py-3 text-right font-medium text-[#666666]">Techo</th>
          ...
          <tbody>
            {presupuestos.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center font-sans text-sm text-[#999999]">
            ...
            {presupuestos.map((p) => {
              const r = resumen(p);
              const fiscal = fiscalForCountry(p.country);
              return (
                <tr
                  key={p.id}
                  onClick={() => router.push(`/presupuesto?plan=${p.id}`)}
                ...
                >
                  ...
                  <td className="px-4 py-3 tabular-nums text-[#666666]">{r.eventoId || "—"}</td>
                  <td className="px-4 py-3 text-[#666666]">{p.country}</td>
                  <td className="px-4 py-3 text-right tabular-nums text-[#333333]">
                    {formatMoney(r.ingreso, fiscal)}
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums text-[#333333]">
                    {formatMoney(r.techo, fiscal)}
                  </td>
...
function NewPresupuestoForm({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await createPresupuestoAction(eventoId);
      if (!res.ok) {
        setError(res.error);
        return;
      }
  ...
  return (
    <div className="rounded-lg border border-[#333333] bg-white p-6">
      <div className="mb-4 flex items-start justify-between">
        <h3 className="font-display text-lg font-bold text-[#333333]">Nuevo presupuesto</h3>
        <button
          type="button"
          onClick={onClose}
          aria-label="Cerrar"
      ...
      {eventos.length === 0 ? (
        <p className="font-sans text-sm text-[#666666]">
          No hay eventos en glovox.categoriaEvento sin presupuesto asignado. Cargá el evento en
          esa tabla o revisá si ya todos tienen presupuesto.
        </p>
      ) : (
        <>
          ...
          <p className="mt-2 font-sans text-xs text-[#999999]">
            Al crear, se siembran asistentes, per-cápitas y % por categoría desde eventos
            comparables del histórico.
          </p>
        </>
      )}
