import type { ProveedorKpis } from "@/lib/queries/proveedor";
import {
  compactCurrency,
  formatCurrency,
  formatNumber,
...
interface Props {
  kpis: ProveedorKpis;
  /** Cuando hay un proveedor seleccionado, el spotlight se titula distinto. */
  scopeLabel: string;
}

export default function KpiRow({ kpis, scopeLabel }: Props) {
  const periodo =
  ...
  const cards = [
    ...
    },
    {
      ...
      value: compactCurrency(kpis.ticketPromedio),
      caption: "Gasto por documento",
    },
  ];
